...
sys.path.insert(0,'/sdcard/mat lab/Conic/CoordGeo')         #path to my scripts
...
eta = u@P[:,0]
a = np.block((u.T + eta*P[:,0].T, V))
b = np.block((-f, eta*P[:,0]-u)) 
...
#if using termux
plt.savefig('/sdcard/mat lab/Conic/CoordGeo/cone.pdf')
subprocess.run(shlex.split("termux-open /sdcard/mat lab/Conic/CoordGeo/cone.pdf"))
#else
#plt.legend()
#plt.show()
